...
public class Heuristic  {
	 ...
	 */
	public Heuristic(Puzzle puzzle) {
		this.puzzle = puzzle;
		this.numCars = this.puzzle.getNumVehiculos();
	...
	private ArrayList<Integer> getInitialBlockingCars() {		
		ArrayList<Integer> blocking = new ArrayList<Integer>();
                ...
                Vehicle objCar = puzzle.getObjectiveCar();
                
		int carSize = puzzle.getCarSize(0);
		int carPos = state.getVariablePosition(0);
		int carFixed = puzzle.getFixedPosition(0);
		for (Vehicle vehiculo : puzzle.cars) {
                        if (objCar.isHorizontal() == vehiculo.isHorizontal()) {
				continue;
			}
			
                        if(objCar.isHorizontal()){
                            if(vehiculo.isHorizontal()){
                                int iFixed = objCar.posY;
			...
			if (carFixed >= iPos && carFixed < iPosFront) {
				blocking.add(i);
                            ...
                            }else if(vehiculo.isVertical()){
                                
                            }
                            
			}else if(objCar.isVertical()){
                            if(vehiculo.isHorizontal()){
